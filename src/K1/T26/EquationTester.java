...
class EquationProcessor {
    static <T,U> void process(List<T> suppliers, List<Equation<T,U>> equations) {
...
public class EquationTester {
    public static void main(String[] args) {
        ...
        if (testCase == 1) { // Testing with Integer, Integer
            ...
            equations2.add(new Equation<>(
                    ()->inputs.get(0),
                    rez->inputs.stream().mapToDouble(Line::calculateLine).filter(i->i>inputs.get(0).calculateLine()).sum()
            ));
