...
class RuleProcessor {
    public static <T,U> void process(List<T> list, List<Rule<T,U>> rules) {
        list.forEach(predicate->{
            ...
            rules.forEach(rule->{
                if(rule.apply(predicate)==Optional.empty()) System.out.println("Condition not met");
                else System.out.println("Result: "+rule.apply(predicate).get());
            });
        });
    }
}
    ...
    public static void main(String[] args) {
        ...
        if (testCase == 1) { //Test for String,Integer
            ...
            * */
            ...
            rules.add(new Rule<>(string -> string.contains("NP"),string -> string.indexOf("NP")));

            /*
            TODO: Add a rule where if the string starts with the string "NP", the result would be length of the string
            * */
            rules.add(new Rule<>(string -> string.startsWith("NP"), String::length));
            ...
            rules.add(new Rule<>(student -> student.grades.size()>=3,student -> student.grades.stream().mapToDouble(i->i).max().orElse(0)));
