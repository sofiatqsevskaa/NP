...
class EvaluatorBuilder {
    public static Evaluator build(Evaluator.TYPE type) throws InvalidEvaluation {
        ...
        return applicant -> {
            if(type==Evaluator.TYPE.NO_CRIMINAL_RECORD) return !applicant.hasCriminalRecord();
            else if(type==Evaluator.TYPE.MORE_EXPERIENCE) return applicant.getEmploymentYears()>=10;
            else if(type==Evaluator.TYPE.MORE_CREDIT_SCORE) return applicant.getCreditScore()>=500;
            else if(type==Evaluator.TYPE.NO_CRIMINAL_RECORD_AND_MORE_EXPERIENCE) return !applicant.hasCriminalRecord() && applicant.getEmploymentYears()>=10;
            else if(type==Evaluator.TYPE.MORE_EXPERIENCE_AND_MORE_CREDIT_SCORE) return applicant.getEmploymentYears()>=10 && applicant.getCreditScore()>=500;
            else return !applicant.hasCriminalRecord()&&applicant.getCreditScore()>500;
        };
    }
}
