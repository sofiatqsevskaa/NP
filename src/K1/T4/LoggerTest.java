...
interface LogProcessor<E extends ILog & Comparable<E>> {
    ArrayList<E> processLogs(ArrayList<E> logs);
}

class LogSystem<E extends ILog & Comparable<E>> {
    ...
    void printResults() {
        ...
        LogProcessor<E> firstLogProcessor = x -> x.stream().filter(i -> i.getType().equals("INFO")).collect(Collectors.toCollection(ArrayList::new));
        LogProcessor<E> secondLogProcessor = x -> x.stream().filter(i -> i.getMessage().length() < 100).collect(Collectors.toCollection(ArrayList::new));
        LogProcessor<E> thirdLogProcessor = x -> x.stream().sorted().collect(Collectors.toCollection(ArrayList::new));
